-- Make the trigger function SECURITY DEFINER to bypass RLS when inserting notifications for other users
  ...
  IF v_instalacao_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      jsonb_build_object(
        ...
        'message', NEW.message,
        'user_id', NEW.user_id
        -- 'data' removed, simplified directly into jsonb
      ),
      false
...
-- Ensure RLS allows the user to read their own notifications
DROP POLICY IF EXISTS "notifications_own" ON public.notifications;
CREATE POLICY "notifications_own" ON public.notifications FOR SELECT USING (user_id = auth.uid());

-- Optional: Allow users to mark their own notifications as read (UPDATE)
