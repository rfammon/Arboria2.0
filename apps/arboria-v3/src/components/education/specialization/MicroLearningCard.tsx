...
export function MicroLearningCard({ title, description, mediaType, mediaSrc, onComplete }: MicroLearningCardProps) {
    ...
    return (
        <Card className="w-full max-w-sm h-[600px] flex flex-col overflow-hidden mx-auto shadow-lg hover:shadow-xl transition-shadow border-2 border-transparent hover:border-primary/20 bg-white dark:bg-slate-900">
            {/* Visual-First Area (approx 70% height) */}
            <div className="relative h-[70%] bg-slate-100 dark:bg-black overflow-hidden">
                {mediaType === 'image' ? (
                    <img src={mediaSrc} alt={title} className="w-full h-full object-cover" />
                ) : (
                    <div className="w-full h-full flex items-center justify-center bg-slate-200 dark:bg-gray-900 group cursor-pointer">
            ...
            <div className="h-[30%] flex flex-col justify-between p-5">
                <div>
                    <h3 className="text-xl font-bold leading-tight mb-2 text-slate-900 dark:text-white">{title}</h3>
